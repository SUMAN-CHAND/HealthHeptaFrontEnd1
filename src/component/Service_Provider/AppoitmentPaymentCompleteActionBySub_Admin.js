...
export default function AppoitmentPaymentCompleteActionBySub_Admin() {
    //main for connecting backend with Session
    axiosClient.defaults.withCredentials = true;
    const [payment, setPayment] = useState([]);
    // const [deliveryPartners, setDeliveryPartner] = useState([]);
    const param = useParams();
    var appoiment_id = param.id;
    useEffect(() => {
        axiosClient.get(`/superadmin/payment/appoiment/status/${appoiment_id}`)
            .then(res => {
    ...
    const handleSubmit = (event) => {
        ...
        axiosClient.post(`/sub-admin/update/appoiment/payment`, values)
            .then(res => {
                if (res.data === 'success') {
                ...
                <div className='bg-white m-3 pt-3 pl-2 rounded w-30 shadow' style={{ minWidth: '40vw', height: '110%' }}>
                    <form action='submit' onSubmit={handleSubmit} style={{ padding: '1vw' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'flex-start', flexDirection: 'column', marginLeft: '5vw' }}>
                            ...
                            <h5 className="font-size-15">Appoiment Booking No: {payment && payment.appoiment_id} </h5>
