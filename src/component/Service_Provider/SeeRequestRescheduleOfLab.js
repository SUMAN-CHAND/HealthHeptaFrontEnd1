...
useEffect(() =>{
    axios.get(`http://localhost:8081/sub-admin/reschedule/lab/see/${appoiment_id}`)
                .then(res => {
                    if (res.data !== null) {
...
// setValues({appoint_date : appoint_date,appoint_time:appoint_time})
    ...
    const handleSubmit = (event) => {
        console.log(values)
            ...
            axios.post(`http://localhost:8081/sub-admin/reschedule/lab/see/${appoiment_id}`,values)
                .then(res => {
                    if (res.data !== null) {
                        // Notify admins and super admins about the new order
                        // socket.emit('newOrder', 'New order placed!');
