                    ...
                    <div className='product-details col-6 m-1' style={{ display: 'flex' }}>
                        <div className="col-5 mt-3 " style={{ marginLeft: '1vw' }}>
                            <div className="productimage m-2" style={{ backgroundColor: 'white', borderRadius: '5px', padding: '2px 0px 43px 0px' }}>
                                ...
                                {image.map(img => (
                                    <img src={`http://${process.env.REACT_APP_HOST}:8081/${img.path}`} alt="" style={{ width: '19vw', padding: '2vw' }} />
                                ))}
                                <div className='spall-image mt-3 m-2'>
                                {image.map(img => (
                                    <img src={`http://${process.env.REACT_APP_HOST}:8081/${img.path}`} alt="" className='mx-1' style={{ width: '5vw' }} />
                                ))}{image.map(img => (
                                    <img src={`http://${process.env.REACT_APP_HOST}:8081/${img.path}`} alt="" style={{ width: '5vw' }} />
                                ))}{image.map(img => (
                                    <img src={`http://${process.env.REACT_APP_HOST}:8081/${img.path}`} alt="" className='mx-1' style={{ width: '5vw' }} />
                                ))}
