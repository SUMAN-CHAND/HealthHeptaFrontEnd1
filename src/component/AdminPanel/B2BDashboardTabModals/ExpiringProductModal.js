...
export default function ExpiringProductModal({ closeTheModal }) {
    const [products, setProducts] = useState([]);
    const [ind_product_Images, setInd_product_Images] = useState([]);


    useEffect(() => {
        axiosClient.get(`/superadmin/product-details/expiring`)
            .then(response => {
                // Handle response
        ...
        <>
            <div style={customStyles}>
                ...
                <div className="" id="list-products">
                    <span style={{ display: 'flex', justifyContent: 'space-between' }}><h2 className='p-2 mx-3'>|| Expiring Products ||</h2> <Link to='addproduct'><button className='btn btn-primary mx-3 my-2' >Add New</button></Link></span>
                            ...
                            <table className="table table-striped" style={{ height: '70vh' }}>
                                ...
                                <tbody style={{ verticalAlign: 'middle', overflowY: 'auto' }}>
                                    ...
                                    {medicineType.type[0] === 'drug' ? <>
                                        {products && products.filter(productf => productf.DrugOrNot === 'drug').map((product, index) => (
                                            <tr key={index}>
                                                <th scope="row">{index + 1}</th>
                                                <th scope="row">{product.product_id}</th>
                                                <td>{ind_product_Images.map((img) => (
                                    ...
                                    {medicineType.type[0] === 'otc' ? <>
                                        {products && products.filter(productf => productf.DrugOrNot === 'otc').map((product, index) => (
                                            <tr key={index}>
                                                <th scope="row">{index + 1}</th>
                                                <th scope="row">{product.product_id}</th>
                                                <td>{ind_product_Images.map((img) => (
