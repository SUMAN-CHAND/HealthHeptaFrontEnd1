...
export default function B2BPaymentCompleteAction() {
    ...
    useEffect(() => {
        axiosClient.get(`/superadmin/b2b/payment/status/${order_id}`)
            .then(res => {
    ...
    const handleSubmit = (event) => {
        ...
        axiosClient.post(`/superadmin/b2b/update/payment`, values)
            .then(res => {
                if (res.data.status === 'success' && res.data.role === "admin") {
                    alert('Payment  status updated Successfully!!')
                    navigate('/superadmin/b2b', { state: { loggedIn: true } });
                }
                if (res.data.status === 'success' && res.data.role === "b2b_employee") {
                    alert('Payment  status updated Successfully!!')
                ...
                <div className='bg-white m-3 pt-3 pl-2 rounded w-30 shadow' style={{ minWidth: '40vw', height: '110%' }}>
                    <form action='submit' onSubmit={handleSubmit} style={{ padding: '1vw' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'flex-start', flexDirection: 'column', marginLeft: '5vw' }}>
                            ...
                            <h5 className="font-size-15">Paid Amount: {payment && payment.paid_amount} </h5>
                            <h5 className="font-size-15">Due Amount: {payment && payment.due_amount} </h5>
                            <h5 className="font-size-15">Payment Type: {payment && payment.payment_type} </h5>
                        </div>
                        <div className=' p-1' style={{ textAlign: 'initial', fontWeight: '700' }} >
                            <label className='p-1' htmlFor="orderstatus">Total Payed Amount: </label>
                            <br />
                            <input
                                className='m-2 p-1'
                                type='decimal'
